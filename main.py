...
print("SVC attributes:", dir(svc))
try:
    print("SVC feature names:", svc.feature_names_in_)  # Check if feature names are accessible
except AttributeError:
    print("Feature names not directly accessible. Using manual alignment.")
...
diseases_list = {15: 'Fungal infection', 4: 'Allergy', 16: 'GERD', 9: 'Chronic cholestasis', 14: 'Drug Reaction',
                 ...
                 25: 'Hypoglycemia', 31: 'Osteoarthritis', 5: 'Arthritis',
                 0: '(vertigo) Paroxysmal Positional Vertigo', 2: 'Acne', 38: 'Urinary tract infection', 35: 'Psoriasis',
                 27: 'Impetigo'}
...
def get_predicted_value(patient_symptoms):
    ...
    valid_symptoms = []
    print("Symptoms dict keys:", list(symptoms_dict.keys()))  # Debug: All valid keys
    print("Received patient symptoms:", patient_symptoms)     # Debug: Raw input
    for item in patient_symptoms:
        item = item.strip().lower().replace(" ", "_").replace("-", "_").replace("__", "_")  # Normalize to underscores
        # Map to trained feature names if they differ
        item_mapping = {
            'foul_smell_of_urine': 'foul_smell_of urine',
            'spotting_urination': 'spotting_ urination'
        }
        item = item_mapping.get(item, item)  # Use mapped name if exists, else original
        print(f"Checking symptom: '{item}'")
    ...
    print("Input vector:", input_vector)  # Debug: Full vector
    if not valid_symptoms:
        return "No valid symptoms provided"
    if sum(input_vector) < 1:
        return "Insufficient valid symptoms for prediction"
    
    # Convert to a DataFrame with feature names to match training data
    import pandas as pd
    feature_names = list(symptoms_dict.keys())
    input_df = pd.DataFrame([input_vector], columns=feature_names)
    print("Input DataFrame:", input_df)  # Debug: Input as DataFrame
    
    prediction = svc.predict(input_df)[0]
    print(f"Prediction raw value: {prediction}, Mapped disease: {diseases_list.get(prediction, 'Unknown disease')}")
    return diseases_list.get(prediction, "Unknown disease")

# Routes
@app.route("/")
def index():
    valid_symptoms = list(symptoms_dict.keys())  # Pass all valid symptoms to the template
...
def home():
    if request.method == 'POST':
        ...
        print("Raw form input:", symptoms)  # Debug: Raw form data
        if not symptoms or symptoms.strip() == "Symptoms":
            message = "Please enter valid symptoms."
            return render_template('index.html', message=message, valid_symptoms=list(symptoms_dict.keys()))
        else:
            # Split and clean the user's input
            user_symptoms = [s.strip().lower().replace(" ", "_").replace("-", "_").replace("__", "_") for s in symptoms.split(',')]
            user_symptoms = [symptom for symptom in user_symptoms if symptom]  # Remove empty strings
            print("Processed symptoms:", user_symptoms)  # Debug: Processed symptoms
            predicted_disease = get_predicted_value(user_symptoms)
            if predicted_disease in ["No valid symptoms provided", "Insufficient valid symptoms for prediction"]:
                message = "None or insufficient valid symptoms recognized. Please use symptoms from the dropdown or typeahead."
                return render_template('index.html', message=message, valid_symptoms=list(symptoms_dict.keys()))
            elif predicted_disease == "Unknown disease":
                message = "An error occurred while predicting the disease. Please try again or contact support."
                return render_template('index.html', message=message, valid_symptoms=list(symptoms_dict.keys()))
            ...
            for i in precautions[0]:
                my_precautions.append(i) if i else None

            return render_template('index.html', predicted_disease=predicted_disease, dis_des=dis_des,
                                 my_precautions=my_precautions, medications=medications, my_diet=rec_diet,
                                 workout=workout, valid_symptoms=list(symptoms_dict.keys()))

    return render_template('index.html', valid_symptoms=list(symptoms_dict.keys()))
...
if __name__ == '__main__':
    print("Initial symptoms_dict length:", len(symptoms_dict))  # Debug: Dictionary size
    print("Initial diseases_list length:", len(diseases_list))  # Debug: Diseases size
    app.run(debug=True)